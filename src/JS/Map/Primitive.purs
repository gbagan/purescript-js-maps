...
import Control.Monad.ST.Uncurried (STFn4, runSTFn4)
...
import JS.Map.Primitive.ST (STMap)
import JS.Map.Primitive.ST as MST
...
thawST :: forall r k v. Map k v -> ST r (STMap r k v)
thawST = _copyST

-- | Convert a mutable Map into an immutable Map
freezeST :: forall r k v. STMap r k v -> ST r (Map  k v)
freezeST = _copyST
...
foreign import runST :: forall k v. (forall r. ST r (STMap r k v)) -> Map k v
...
singleton k v = runST (MST.poke k v =<< MST.new)
...
insert k v = mutate (MST.poke k v)

-- | Delete a key and value from a map
delete :: forall k v. Key k => k -> Map k v -> Map k v
delete k = mutate (MST.delete k)
...
fromFoldable l = runST do
  s <- MST.new
  ST.foreach (A.fromFoldable l) \(Tuple k v) -> void $ MST.poke k v s
  pure s
...
fromFoldableWithIndex l = runST do
  s <- MST.new
  forWithIndex_ l \k v -> MST.poke k v s
  pure s
...
fromFoldableWith f l = runST (do
  s <- MST.new
  for_ l \(Tuple k v) ->
    runSTFn4 _lookupST v (f v) k s >>= \v' -> MST.poke k v' s
  pure s)
...
union m = mutate (\s -> foldM (\s' k v -> MST.poke k v s') s m)

-- | Compute the union of two maps, using the specified function
-- | to combine values for duplicate keys.
unionWith :: forall k v. Key k => (v -> v -> v) -> Map k v -> Map k v -> Map k v
unionWith f m1 m2 =
  mutate (\s1 -> foldM (\s2 k v1 -> MST.poke k (runFn4 _lookup v1 (\v2 -> f v1 v2) k m2) s2) s1 m1) m2
...
filterWithKey predicate m = runST go
  ...
  go = do
    ...
    foldM step m' m
    where
      step acc k v = if predicate k v then MST.poke k v acc else pure acc
